...
class NewUserCrawlerSpider(scrapy.Spider):
    ...
    def __init__(self, kor_file="test/user_log.dat",
                 ...
                 new_kor_path="test/",
                 post_urls_file='test/post_urls.dat'):
        ...
        self.post_urls = loadPostUrls(post_urls_file)

    def start_requests(self):
        for idx, url in enumerate(self.post_urls):
            yield Request(url, meta={'seq': idx})
            if idx % 1000 == 0:
                print idx
    ...
    def parse_profile_page(self, response):
        ...
        if not temp:  # user["is_private"] or
            return
        nodes = temp[0]
...
def loadPostUrls(urls_filename):
...
def random_key(length):
    alphanu = string.lowercase + string.uppercase + string.digits
    return ''.join(random.choice(alphanu) for _ in range(length))

# key = random_key(32)

# headers = {
#     # csrftoken cookies just have to match with x-csrftoken
...
data = {
  # after number is some arbitrarily chosen number for now
  'q': '''ig_user(%s) { media.after(9379061665064369409, 12) {
  nodes {
    caption,
    code
